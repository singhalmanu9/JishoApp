...
void main() {
  ...
  Map JSONAnswers =
      jsonDecode(File('assets/json_files/answerMap.json').readAsStringSync());
...
///Splits a Trie from one large data structure to smaller
///clusters of MAPSIZE nodes. This is done to shorten load times
///during searches.
void splitTrie(Trie root, String initpath) {
  ...
  while (cur.isNotEmpty) {
    ...
    val.c.forEach((key, child) {
      if (child is Trie) updated_c[key] = child.id;
      cur.add(child);
    });
    val.c = updated_c;
